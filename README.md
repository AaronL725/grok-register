...
临时邮箱提供商[迈巢](https://mailnest.top/)，针对临时邮箱采用项目制模式，可以在某一特定项目中使用临时邮箱，因此价格低廉，配置方便。全网价格最低，临时邮箱仅`0.018`元一个，收件成功才扣款，未成功收件`20min`后过期自动解冻。

仅需要在配置文件中配置`api-key`与项目代码`mailnest_project_code`，项目代码默认为`x-ai001`:
...
配置后，在[充值页面](https://mailnest.top/recharges)完成充值后，即可在`qt`界面的邮箱服务商选择`mailnest`进行使用。
