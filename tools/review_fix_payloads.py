...
def validate_config(raw_config):
    ...
    if not values["user_agent"]:
        _config_error("user_agent", "不能为空")
    if not values["cpa_auth_dir"]:
        _config_error("cpa_auth_dir", "不能为空")
...
def _legacy_remote_pool_replace(api_bases, base, headers, query, pool_name, token, email, log_callback=None):
    ...
    for api_base in api_bases or [base]:
        ...
        current = payload.get("tokens") if isinstance(payload, dict) and "tokens" in payload else payload
        ...
        try:
            saved = http_post(endpoint, headers=save_headers, params=query, json=current, timeout=30)
        except Exception as exc:
        ...
        if save_status in (409, 412):
            raise RemoteTokenPoolRequestError(f"远端 token 池写入发生并发冲突，If-Match 被拒绝 HTTP {save_status}")
...
START_REGISTRATION = r'''    def start_registration(self):
        if self.is_running:
            self.log("[!] 当前已有任务在运行")
            return
        ...
        raw_paths = [item.strip() for item in self.cloudflare_paths_var.get().split(",") if item.strip()]
        if len(raw_paths) >= 4:
        ...
        threading.Thread(target=self.run_registration, args=(count,), daemon=True).start()
'''
