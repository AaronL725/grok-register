(() => {
  ...
  const zh = {
    ...
    proxyEmpty:'暂无代理节点', proxyNode:'节点', proxyRunHealth:'运行健康分', proxyProbeStatus:'探测状态',
  ...
  Object.assign(i18n.en.fields, {
    ...
    proxy_protocol_backend:['Advanced protocol backend','auto normalizes native proxies to a common HTTP endpoint and routes advanced protocols through sing-box; native-only disables advanced protocols.'],
  ...
  function probeText(status) {
    if (status === 'healthy') return t('probeHealthy');
    if (status === 'unhealthy') return t('probeUnhealthy');
    if (status === 'unavailable') return t('probeUnavailable');
    return t('probeUnknown');
  }
  function renderProxyStatus(data) {
    ...
    if (!nodes.length) {
      rows.innerHTML = `<tr><td colspan="12" class="proxy-empty">${esc(t('proxyEmpty'))}</td></tr>`;
      return;
    }
    rows.innerHTML = nodes.map(node => {
      ...
      const samples = Number(node.business_samples || 0);
      const health = samples > 0 ? `${node.health} · n=${samples}` : `— · ${t('noBusinessSamples')}`;
      const latency = node.probe_status === 'healthy'
        ? (node.probe_latency_ms ? `${node.probe_latency_ms} ms` : '—')
        : (node.probe_latency_ms ? `${node.probe_latency_ms} ms ${t('failedAfter')}` : '—');
      ...
      return `<tr>
        ...
        <td>${node.rotating ? 'rotating' : 'fixed'}</td><td>${esc(probeText(node.probe_status))}</td>
